#!/usr/bin/env python3
"""
CLI Pulse v1.10.7 — ASC metadata + submit.

What this does, per platform (iOS + macOS):
  1. Find the newest editable appStoreVersion (PREPARE_FOR_SUBMISSION /
     DEVELOPER_REJECTED / INVALID_BINARY / METADATA_REJECTED / etc.). If
     that version is still on 1.10.6, bump its versionString to 1.10.7.
     If the only in-flight version is WAITING_FOR_REVIEW on 1.10.6, cancel
     its reviewSubmission first and flip versionString to 1.10.7.
  2. Wait for build 41 to reach processingState=VALID, then bind it.
  3. Patch en-US whatsNew with the v1.10.7 release notes.
...
TARGET_VERSION = "1.10.8"
TARGET_BUILD = "43"

WHATS_NEW_EN = """v1.10.8 reliability + alerts hardening:
