...
#   1. the real pre-v0.70 schema/RPC baseline plus migrate_v0.70;
...
#
# Usage:
#   UPGRADE_DATABASE_URL=postgres://.../v070_upgrade \
#   FRESH_DATABASE_URL=postgres://.../v070_fresh \
#     ./backend/supabase/tests/run_v070_provider_accounts_tests.sh
set -euo pipefail

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(git -C "$HERE" rev-parse --show-toplevel)"
# Direct parent of the first v0.70 backend commit (0ff120e). Pinning the exact
# parent makes the upgrade fixture follow branch history without guessing from
# a release tag or a nearby commit.
BASELINE_REF="${PROVIDER_ACCOUNTS_BASE_REF:-e80584443085725380df371bb4b3d326244b28f6}"
...
if ! git -C "$REPO_ROOT" cat-file -e "${BASELINE_REF}^{commit}"; then
  echo "pre-v0.70 baseline commit is unavailable: $BASELINE_REF" >&2
  echo "fetch full history or set PROVIDER_ACCOUNTS_BASE_REF explicitly" >&2
  exit 2
fi

BASELINE_DIR="$(mktemp -d "${TMPDIR:-/tmp}/clipulse-v070-baseline.XXXXXX")"
cleanup() {
  rm -rf "$BASELINE_DIR"
}
trap cleanup EXIT
...
echo "== provider accounts: pre-v0.70 upgrade path =="
seed_supabase_shim "$UPGRADE_DATABASE_URL"
apply_sql "$UPGRADE_DATABASE_URL" "loading pre-v0.70 schema" \
  "$BASELINE_DIR/schema.sql"
apply_sql "$UPGRADE_DATABASE_URL" "loading pre-v0.70 app RPCs" \
  "$BASELINE_DIR/app_rpc.sql"
apply_sql "$UPGRADE_DATABASE_URL" "loading pre-v0.70 helper RPCs" \
  "$BASELINE_DIR/helper_rpc.sql"
apply_sql "$UPGRADE_DATABASE_URL" "applying migrate_v0.70" \
  "$REPO_ROOT/backend/supabase/migrate_v0.70_provider_accounts.sql"
apply_sql "$UPGRADE_DATABASE_URL" "running upgrade-path contract assertions" \
  "$HERE/migrate_v0.70_provider_accounts.test.sql"
DATABASE_URL="$UPGRADE_DATABASE_URL" \
  bash "$HERE/migrate_v0.70_provider_accounts.concurrent.sh"
...
echo "provider-account v0.70 upgrade + fresh contract suites: PASS"
